(() => {
  ...
  var require_Global = __commonJS({
    "node_modules/tstl/internal/Global.js"(exports) {
      ...
      function _Get_root() {
        if (__s_pRoot === null) {
          __s_pRoot = (0, node_1.is_node)() ? self : self;
  ...
  var require_HashBuckets = __commonJS({
    "node_modules/tstl/internal/hash/HashBuckets.js"(exports) {
      ...
      var HashBuckets = function() {
            ...
            for (var _c = __values(this.data_), _d = _c.next(); !_d.done; _d = _c.next()) {
              ...
              try {
                for (var row_1 = (e_2 = void 0, __values(row)), row_1_1 = row_1.next(); !row_1_1.done; row_1_1 = row_1.next()) {
                  var elem2 = row_1_1.value;
                  var index = this.hasher_(this.fetcher_(elem2)) % data.length;
                  data[index].push(elem2);
                }
              } catch (e_2_1) {
                e_2 = { error: e_2_1 };
              } finally {
        ...
        HashBuckets2.prototype.index = function(elem2) {
          return this.hasher_(this.fetcher_(elem2)) % this.length();
        };
  ...
  init_virtual_process_polyfill();
  init_buffer();
  function elem(name = "div", { data, ...props } = {}, children = []) {
  ...
  init_virtual_process_polyfill();
  init_buffer();
  var dateTime = new Intl.DateTimeFormat("de-ch", {
    dateStyle: "medium",
    timeStyle: "short"
  });
  var timeAgo = (time, locale = "en") => {
    const relativeTime = new Intl.RelativeTimeFormat(locale, {
      numeric: "auto",
      style: "long"
    });
    const timeSince = (Date.now() - time.getTime()) * 1e-3;
  ...
  function onEvent(evt2, relay) {
    switch (evt2.kind) {
      case 0:
        handleMetadata(evt2, relay);
        break;
      case 1:
        handleTextNote(evt2, relay);
        break;
      case 2:
        handleRecommendServer(evt2, relay);
        break;
      case 3:
        break;
      case 7:
        handleReaction(evt2, relay);
      default:
    }
  }
  var pubkey = localStorage.getItem("pub_key") || (() => {
    const privatekey = generatePrivateKey();
    const pubkey2 = getPublicKey(privatekey);
    localStorage.setItem("private_key", privatekey);
    localStorage.setItem("pub_key", pubkey2);
    return pubkey2;
  })();
  var subList = [];
  var unSubAll = () => {
  ...
  function sub24hFeed() {
    subList.push(pool.sub({
      ...
      filter: {
        kinds: [0, 1, 2, 7],
        since: Math.floor(Date.now() * 1e-3 - 24 * 60 * 60),
        limit: 450
      }
    }));
  }
  ...
  function subProfile(pubkey2) {
    subList.push(pool.sub({
      cb: (evt2, relay) => {
        renderProfile(evt2, relay);
        showProfileDetail();
      },
      filter: {
        authors: [pubkey2],
  ...
  var profileAbout = profileContainer.querySelector(".profile-about");
  var profileName = profileContainer.querySelector(".profile-name");
  var profilePubkey = profileContainer.querySelector(".profile-pubkey");
  var profilePubkeyLabel = profileContainer.querySelector(".profile-pubkey-label");
  var profileImage = profileContainer.querySelector(".profile-image");
  var textNoteContainer = document.querySelector("#textnote");
  ...
  function renderProfile(evt2, relay) {
    profileContainer.dataset.pubkey = evt2.pubkey;
    profilePubkey.textContent = evt2.pubkey;
    profilePubkeyLabel.hidden = false;
    const content = parseContent(evt2.content);
    if (content) {
      profileAbout.textContent = content.about;
      profileName.textContent = content.name;
      const noxyImg = getNoxyUrl("data", content.picture, evt2.id, relay);
      if (noxyImg) {
        profileImage.setAttribute("src", getNoxyUrl("data", noxyImg, evt2.id, relay));
  ...
  function showTextNoteDetail(evt2, relay) {
    if (!textNoteContainer.querySelector(`[data-id="${evt2.id}"]`)) {
      textNoteContainer.append(createTextNote(evt2, relay));
    }
    textNoteContainer.hidden = false;
  ...
  document.body.addEventListener("click", (e) => {
    ...
    const pubkey2 = e.target.closest("[data-pubkey]")?.dataset.pubkey;
    ...
    if (username) {
      history.pushState({ author: pubkey2 }, "", `/${pubkey2}`);
      unSubAll();
      clearProfile();
      clearTextNoteDetail();
      subProfile(pubkey2);
  ...
  var textNoteList = [];
  var eventRelayMap = {};
  var hasEventTag = (tag) => tag[0] === "e";
  function handleTextNote(evt2, relay) {
    if (eventRelayMap[evt2.id]) {
      eventRelayMap[evt2.id] = [relay, ...eventRelayMap[evt2.id]];
    } else {
      eventRelayMap[evt2.id] = [relay];
      if (evt2.tags.some(hasEventTag)) {
        handleReply(evt2, relay);
      } else {
        textNoteList.push(evt2);
      }
      renderFeed();
    }
  }
  var replyList = [];
  var reactionMap = {};
  var getReactionList = (id) => {
    return reactionMap[id]?.map(({ content }) => content) || [];
  };
  function handleReaction(evt2, relay) {
    if (!evt2.content.length) {
      return;
    }
    const eventTags = evt2.tags.filter(hasEventTag);
    let replies = eventTags.filter(([tag2, eventId2, relayUrl, marker]) => marker === "reply");
    if (replies.length === 0) {
      replies = eventTags.filter((tags) => tags[3] === void 0);
    }
    if (replies.length !== 1) {
      console.log("call me", evt2);
      return;
    }
    const [tag, eventId] = replies[0];
    if (reactionMap[eventId]) {
      if (reactionMap[eventId].find((reaction) => reaction.id === evt2.id)) {
        return;
      }
      reactionMap[eventId] = [evt2, ...reactionMap[eventId]];
    } else {
      reactionMap[eventId] = [evt2];
    }
  ...
  var feedContainer = document.querySelector("#homefeed");
  var feedDomMap = {};
  var replyDomMap = {};
  var restoredReplyTo = localStorage.getItem("reply_to");
  var sortByCreatedAt = (evt1, evt2) => {
  ...
  function renderFeed() {
    ...
    sortedFeeds.forEach((evt2, i) => {
      if (feedDomMap[evt2.id]) {
        return;
      }
      const article = createTextNote(evt2, eventRelayMap[evt2.id]);
      if (i === 0) {
        feedContainer.append(article);
      } else {
        feedDomMap[sortedFeeds[i - 1].id].before(article);
      }
      feedDomMap[evt2.id] = article;
    });
  }
  setInterval(() => {
  ...
  var getNoxyUrl = (type, url, id, relay) => {
    if (!isHttpUrl(url)) {
      return false;
    }
  ...
  var fetchNext = (href, id, relay) => {
    ...
    fetchPending = fetch(noxy.href).then((data) => {
      if (data.status === 200) {
        return data.json();
      }
      ...
      fetchPending = false;
      if (fetchQue.length) {
        const { href: href2, id: id2, relay: relay2 } = fetchQue.shift();
        return fetchNext(href2, id2, relay2);
      }
    }).catch((err) => err.text && err.text()).then((errMsg) => errMsg && console.warn(errMsg));
    return previewId;
  };
  ...
  function createTextNote(evt2, relay) {
    const { host, img, isReply, name, replies, time, userName } = getMetadata(evt2, relay);
    const hasReactions = reactionMap[evt2.id]?.length > 0;
    const didReact = hasReactions && !!reactionMap[evt2.id].find((reaction) => reaction.pubkey === pubkey);
    const replyFeed = replies[0] ? replies.map((e) => replyDomMap[e.id] = createTextNote(e, relay)) : [];
    const [content, { firstLink }] = parseTextContent(evt2.content);
...
User pubkey: ${evt2.pubkey}

Relay: ${host}

Event-id: ${evt2.id}
      ${evt2.tags.length ? `
Tags ${JSON.stringify(evt2.tags)}
` : ""}
      ${isReply ? `
Reply to ${evt2.tags[0][1]}
` : ""}
      ${evt2.content}`
      }, [
        elem("small", {}, [
      ...
      elem("div", {}, [
        ...content,
        firstLink ? linkPreview(firstLink, evt2.id, relay) : ""
      ]),
      elem("button", {
        className: "btn-inline",
        name: "star",
        type: "button",
        data: { "eventId": evt2.id, relay }
      }, [
        elem("img", {
          alt: didReact ? "\u272D" : "\u2729",
          height: 24,
          ...
          title: getReactionList(evt2.id).join(" ")
        }),
        elem("small", { data: { reactions: evt2.id } }, hasReactions ? reactionMap[evt2.id].length : "")
      ]),
      elem("button", {
        className: "btn-inline",
        name: "reply",
    ...
    ], { data: { id: evt2.id, pubkey: evt2.pubkey, relay } });
  }
  function handleReply(evt2, relay) {
    if (replyDomMap[evt2.id]) {
      console.log("CALL ME already have reply in replyDomMap", evt2, relay);
      return;
    }
    replyList.push(evt2);
    renderReply(evt2, relay);
  }
  function renderReply(evt2, relay) {
    const eventId = evt2.tags[0][1];
    const article = feedDomMap[eventId] || replyDomMap[eventId];
    if (!article) {
      return;
    }
    ...
    const reply = createTextNote(evt2, relay);
    replyContainer.append(reply);
    replyDomMap[evt2.id] = reply;
  }
  var sortEventCreatedAt = (created_at) => ({ created_at: a }, { created_at: b }) => Math.abs(a - created_at) < Math.abs(b - created_at) ? -1 : 1;
  function handleRecommendServer(evt2, relay) {
    if (feedDomMap[evt2.id]) {
      return;
    }
    const art = renderRecommendServer(evt2, relay);
    if (textNoteList.length < 2) {
      feedContainer.append(art);
      return;
    }
    const closestTextNotes = textNoteList.sort(sortEventCreatedAt(evt2.created_at));
    feedDomMap[closestTextNotes[0].id].after(art);
    feedDomMap[evt2.id] = art;
  }
  function renderRecommendServer(evt2, relay) {
    const { img, name, time, userName } = getMetadata(evt2, relay);
    const body = elem("div", { className: "mbox-body", title: dateTime.format(time) }, [
      ...
      ` recommends server: ${evt2.content}`
    ]);
    return renderArticle([
      elem("div", { className: "mbox-img" }, [img]),
      body
    ], { className: "mbox-recommend-server", data: { id: evt2.id, pubkey: evt2.pubkey } });
  }
  ...
  function handleMetadata(evt2, relay) {
    const content = parseContent(evt2.content);
    if (content) {
      setMetadata(evt2, relay, content);
    }
  }
  function setMetadata(evt2, relay, content) {
    let user = userList.find((u) => u.pubkey === evt2.pubkey);
    const picture = getNoxyUrl("data", content.picture, evt2.id, relay).href;
    if (!user) {
      user = {
    ...
    if (user.picture) {
      document.body.querySelectorAll(`canvas[data-pubkey="${evt2.pubkey}"]`).forEach((canvas) => canvas.parentNode.replaceChild(elem("img", { src: user.picture }), canvas));
    }
    if (user.metadata[relay].name) {
      document.body.querySelectorAll(`[data-id="${evt2.pubkey}"] .mbox-username:not(.mbox-kind0-name)`).forEach((username) => {
  ...
  function getMetadata(evt2, relay) {
    const host = getHost(relay);
    const user = userList.find((user2) => user2.pubkey === evt2.pubkey);
    const userImg = user?.picture;
    const name = user?.metadata[relay]?.name;
    const userName = name || evt2.pubkey.slice(0, 8);
    ...
    }) : elemCanvas(evt2.pubkey);
    const isReply = evt2.tags.some(hasEventTag);
    const replies = replyList.filter((reply) => reply.tags[0][1] === evt2.id);
    const time = new Date(evt2.created_at * 1e3);
    return { host, img, isReply, name, replies, time, userName };
  }
  var writeForm = document.querySelector("#writeForm");
  var writeInput = document.querySelector('textarea[name="message"]');
  var elemShrink = () => {
  ...
  writeInput.addEventListener("focusout", () => {
    ...
    if (reply_to && writeInput.value === "") {
      writeInput.addEventListener("transitionend", (event) => {
        if (!reply_to || reply_to === localStorage.getItem("reply_to") && !writeInput.style.height) {
  ...
  var newMessageDiv = document.querySelector("#newMessage");
  ...
  async function upvote(eventId, relay) {
    ...
    const newReaction = {
      ...
      created_at: Math.floor(Date.now() * 1e-3)
    };
  ...
  var sendStatus = document.querySelector("#sendstatus");
  var onSendError = (err) => sendStatus.textContent = err.message;
  ...
  var settingsForm = document.querySelector('form[name="settings"]');
  var privateKeyInput = settingsForm.querySelector("#privatekey");
  var pubKeyInput = settingsForm.querySelector("#pubkey");
  var statusMessage = settingsForm.querySelector("#keystatus");
  var generateBtn = settingsForm.querySelector('button[name="generate"]');
  var importBtn = settingsForm.querySelector('button[name="import"]');
  var privateTgl = settingsForm.querySelector('button[name="privatekey-toggle"]');
  generateBtn.addEventListener("click", () => {
    const privatekey = generatePrivateKey();
    const pubkey2 = getPublicKey(privatekey);
    if (validKeys(privatekey, pubkey2)) {
      privateKeyInput.value = privatekey;
      pubKeyInput.value = pubkey2;
  ...
  privateKeyInput.addEventListener("paste", (event) => {
    ...
    if (privateKeyInput.value === "" || privateKeyInput.selectionStart === 0 && privateKeyInput.selectionEnd === privateKeyInput.value.length) {
      try {
  ...
  function validKeys(privatekey, pubkey2) {
    try {
      if (getPublicKey(privatekey) === pubkey2) {
  ...
  var profileForm = document.querySelector('form[name="profile"]');
  var profileSubmit = profileForm.querySelector('button[type="submit"]');
  var profileStatus = document.querySelector("#profilestatus");
